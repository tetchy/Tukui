...
if not(C["unitframes"].layout == "Surgency") then return end
if not C["unitframes"].enable == true then return end

print("Surgency layout enabled")
...
local function Shared(self, unit)
	...
	if (unit == "player" or unit == "target") then	
		...
		if (unit == "player") then
			...
			Leader:Height(14)
			Leader:Width(14)
			Leader:Point("TOPLEFT", UIParent, "TOPLEFT", -50, 80)
			self.Leader = Leader
			...
			if C["unitframes"].classbar then
				if T.myclass == "DRUID" then			
					...
					-- eclipseBar:Point("TOP", self, "TOP", 0, -4)
					-- eclipseBar:Size(80, 4)
					-- eclipseBar:SetFrameStrata("MEDIUM")
					-- eclipseBar:SetFrameLevel(panel:GetFrameLevel())
					...
					-- lunarBar:Point('LEFT', eclipseBar, 'LEFT', 1, 0)
					-- lunarBar:Size(eclipseBar:GetWidth()-2, eclipseBar:GetHeight()-2)
					...
					-- solarBar:Size(eclipseBar:GetWidth()-2, eclipseBar:GetHeight()-2)
				...
				if (T.myclass == "WARLOCK" or T.myclass == "PALADIN") then
					...
					bars:SetFrameLevel(panel:GetFrameLevel())
					bars:Point("CENTER", self, "CENTER", 0, 0)
					bars:Size(36, 10)
					
					for i = 1, 3 do					
						bars[i]=CreateFrame("StatusBar", self:GetName().."_Shard"..i, self)
						bars[i]:Height(bars:GetHeight())
						bars[i]:Width(bars:GetHeight())
						bars[i]:SetFrameLevel(bars:GetFrameLevel()+1)
						
						bars[i]:SetStatusBarTexture(normTex)
						bars[i]:GetStatusBarTexture():SetHorizTile(false)

						bars[i].border = CreateFrame("frame", nil,  self)
						bars[i].border:SetFrameLevel(bars[i]:GetFrameLevel()-1)
						bars[i].border:SetBackdrop({bgFile = C.media.blank})
						bars[i].border:SetBackdropColor(0,0,0,1)
						bars[i].border:SetPoint("TOPLEFT", bars[i], "TOPLEFT", -1, 1)
						bars[i].border:SetPoint("BOTTOMRIGHT", bars[i], "BOTTOMRIGHT", 1, -1)
						...
						if T.myclass == "WARLOCK" then
							r,g,b = 140/255, 0/255, 125,255
						elseif T.myclass == "PALADIN" then
							r,g,b = 228/255,225/255,16/255
						end
						bars[i]:SetStatusBarColor(r,g,b)
						bars[i].bg:SetTexture(r,g,b,0.15)
						
						if i == 1 then
							bars[i]:SetPoint("LEFT", bars, "LEFT", 1, 0)
						else
							bars[i]:Point("LEFT", bars[i-1], "RIGHT", 3, 0)
						end
				...
				if T.myclass == "DEATHKNIGHT" then
					...
					Runes:SetFrameLevel(panel:GetFrameLevel())
					Runes:SetPoint("CENTER", self, "CENTER", 0, 0)
					Runes:Size(39, frameHeight-10)
					...
					for i = 1, 6 do
						...
						if (i == 1) then
							...
							Runes[i]:Point("TOPLEFT", Runes[i-1], "TOPRIGHT", 3, 0)
						end
						...
						Runes[i].border = Runes[i]:CreateTexture(nil, "BACKGROUND")
						Runes[i].border:SetTexture(0,0,0)
						Runes[i].border:SetPoint("TOPLEFT", Runes[i], "TOPLEFT", -1, 1)
						Runes[i].border:SetPoint("BOTTOMRIGHT", Runes[i], "BOTTOMRIGHT", 1, -1)
				...
				if T.myclass == "SHAMAN" then
					...
					TotemFrame:SetPoint("CENTER", self, "CENTER", 0, 0)
					TotemFrame:Width(18)
					TotemFrame:Height(frameHeight-10)
					TotemFrame:SetFrameLevel(panel:GetFrameLevel())
					...
					for i = 1, 4 do
						...
						if (i == 1) then
							TotemBar[i]:SetPoint("LEFT", TotemFrame, "LEFT", 0, 0)					
						else
							TotemBar[i]:SetPoint("TOPLEFT", TotemBar[i-1], "TOPRIGHT", TukuiDB.Scale(3), 0)
						end
						TotemBar[i]:SetStatusBarTexture(normTex)
						TotemBar[i]:Height(TotemFrame:GetHeight())
						TotemBar[i]:Width(4)
						...
						TotemBar[i].border = TotemBar[i]:CreateTexture(nil, "BACKGROUND")
						TotemBar[i].border:SetTexture(0,0,0)
						TotemBar[i].border:SetPoint("TOPLEFT", TotemBar[i], "TOPLEFT", -1, 1)
						TotemBar[i].border:SetPoint("BOTTOMRIGHT", TotemBar[i], "BOTTOMRIGHT", 1, -1)
			...
			do
				...
				local ComboFrame = CreateFrame("Frame", self:GetName().."_ComboFrame", self)
				ComboFrame:SetPoint("CENTER", self, "CENTER", 0, 0)
				ComboFrame:Width(62)
				ComboFrame:Height(10)
				ComboFrame:SetFrameLevel(panel:GetFrameLevel())
				local ComboPoint={}
				for i=1, 5 do
					ComboPoint[i] = CreateFrame("StatusBar", self:GetName().."_TotemBar"..i, self)
					if (i == 1) then
						ComboPoint[i]:SetPoint("LEFT", ComboFrame, "LEFT", 0, 0)					
					else
						ComboPoint[i]:SetPoint("TOPLEFT", ComboPoint[i-1], "TOPRIGHT", TukuiDB.Scale(3), 0)
					end
					ComboPoint[i]:SetStatusBarTexture(normTex)
					ComboPoint[i]:Height(ComboFrame:GetHeight())
					ComboPoint[i]:Width(ComboFrame:GetHeight())
					
					ComboPoint[i]:SetFrameLevel(ComboFrame:GetFrameLevel()+1)
					ComboPoint[i]:SetMinMaxValues(0, 1)
					ComboPoint[i]:SetValue(0)
					
					ComboPoint[i].border = CreateFrame("frame", nil,  self)
					ComboPoint[i].border:SetFrameLevel(ComboPoint[i]:GetFrameLevel()-1)
					ComboPoint[i].border:SetBackdrop({bgFile = C.media.blank})
					ComboPoint[i].border:SetBackdropColor(0,0,0,1)
					ComboPoint[i].border:SetPoint("TOPLEFT", ComboPoint[i], "TOPLEFT", -1, 1)
					ComboPoint[i].border:SetPoint("BOTTOMRIGHT", ComboPoint[i], "BOTTOMRIGHT", 1, -1)
					ComboPoint[i].border:Hide()
					
					ComboPoint[i]:SetStatusBarColor(unpack(cpColors[i]))
					
					ComboPoint[i]:RegisterEvent("PLAYER_ENTERING_WORLD")
					ComboPoint[i]:RegisterEvent("UNIT_COMBO_POINTS")
					ComboPoint[i]:RegisterEvent("PLAYER_TARGET_CHANGED")
					ComboPoint[i]:SetScript("OnEvent", function(self, event)
					--Credits to sCombo addon for this script
					local points, pt = 0, GetComboPoints("player", "target")
						...
						elseif pt > points then
							for i = points + 1, pt do
								ComboPoint[i]:SetValue(1)
								ComboPoint[i].border:Show()
							end
						else
			...
			self:SetScript("OnEnter", function(self)
				...
				if status then status:Show() end
		...
		if (C["unitframes"].unitcastbar == true) then
			...
			if C.unitframes.cbposition == true then
				...
				castbar:Point("TOP", powerBorder, "BOTTOM",  0, -3)
				castbar:SetWidth(frameWidth)
				castbar:SetHeight(16)
			end
			
			castbar.border = CreateFrame("frame", nil, castbar)
			castbar.border:SetPoint("TOPLEFT", castbar, "TOPLEFT", -1, 1)
			castbar.border:SetPoint("BOTTOMRIGHT", castbar, "BOTTOMRIGHT", 1, -1)
			castbar.border:SetTemplate("Thin")
			...
			castbar.Text:Point("RIGHT", castbar, "RIGHT", -50, 0)
	...
	if (unit == "pet") or (unit == "pettarget") then
		...
		-- power
		powerBorder = CreateFrame("frame", self:GetName().."_PowerBorder", self)
		powerBorder:CreatePanel("Default", frameWidth, frameHeight, "TOPLEFT", self, "TOPLEFT", -2, 2)
		powerBorder:SetFrameStrata("MEDIUM")
		powerBorder:SetFrameLevel(1)
		powerBorder:SetBackdropColor(0,0,0)
		if unit == "pettarget" then
			powerBorder:ClearAllPoints()
			powerBorder:SetPoint("TOPRIGHT", self, "TOPRIGHT", 2, 2)
		end
		...
		if (C["unitframes"].unitcastbar == true) and (unit=="pet") then
			...
			castbar:SetFrameLevel(6)
			castbar:Point("BOTTOM", powerBorder, "TOP",  0, 3)
			castbar:SetWidth(frameWidth)
...
pet:SetPoint("BOTTOMLEFT", player, "TOPLEFT", 0,5)
