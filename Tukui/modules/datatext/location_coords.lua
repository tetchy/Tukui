...
----- [[    Location    ]] -----
if C["datatext"].location ~= true then 
	return
else
	local location = CreateFrame("Frame", "TukuiLocation", UIParent)
	location:CreatePanel("Default", 60, 20, "CENTER", TukuiTopBar, "BOTTOM", 0, -7)
	location:SetFrameLevel(2)

	local locationtext  = location:CreateFontString(nil, "LOW")
	locationtext:SetFont(C.media.font, C["datatext"].fontsize)
	...
	-- code to hide or show raid markers
	...
	xcoordstext:SetFont(C.media.font, C["datatext"].fontsize)
	xcoordstext:SetPoint("CENTER", xcoords, "CENTER", 1, 0)

	local ycoordstext  = ycoords:CreateFontString(nil, "OVERLAY")
	ycoordstext:SetFont(C.media.font, C["datatext"].fontsize)
	...
	local Update = function(self,t)
		...
		if x == 0 and y == 0 then
			...
			xcoordstext:SetText(hexa..("%2.1f"):format(x*100)..hexb)
			ycoordstext:SetText(hexa..("%2.1f"):format(y*100)..hexb)
		end
		ela = .2
	end
